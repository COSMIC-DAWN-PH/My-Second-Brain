...
以 **canonical dependency graph**（**[references/dependency-graph.md](references/dependency-graph.md)**）为主线依赖关系；正文双链只用于 cross-validation，不覆盖主线依赖。
...
- 2026-08-05: 渐进式拆分——canonical 依赖图移至 references/dependency-graph.md，SKILL.md 保留执行流程
