...
> **⚠️ 证据标准**：文件修改时间只能用于判断"文件是否在扫描范围内被编辑过"，**不能**推断"用户学习了该文件的内容"。只有 block reference 标记才是用户学习的证据。

### Block reference 区间语义
...
以此类推，形成 **已学 → 没懂 → 已学 → 没懂** 的交替区间链。扫描时按顺序读取所有 block reference，据此判断每个章节区间的学习状态。
...
> **链接格式要求**：总结中所有引用具体章节或 block reference 的地方，必须使用 Obsidian 可跳转链接：`[[笔记名#章节标题|显示文本]]` 或 `[[笔记名#^block-id|显示文本]]`。不能用纯文本 `§X` 代替。
...
2. **知识笔记**：从 `Rydberg atom/` 找在扫描范围内修改过的文件，列出名称（`[[英文名|中文名]]` 格式）+ `comprehension` 字段 + 用户标记的学习进度。**没有 block reference 标记的章节一律视为未学**。
3. **链接格式**：引用具体章节必须用 Obsidian 可跳转链接：`[[笔记名#章节标题|显示文本]]` 或 `[[笔记名#^block-id|显示文本]]`。不能用纯文本 `§X` 代替。表格中用 `[[笔记名#章节标题\|显示文本]]` 转义。
