...
2. **目录同步**：`.claude/` 与 `.agents/` 之间的**文件一致性**——双向比较，内容更丰富、更完整的一方胜出。

**核心原则**：
- **无固定权威源**：两边都可能被修改，同步时比较内容，哪个更丰富更完整就用哪个。
- **"更丰富"判定标准**：行数更多、规则声明更详细、包含更多示例或约束条件、覆盖更多边界情况。当难以判定时，暂停让用户选择。
...
|------|------|
| 两边都存在且内容相同 | 跳过 |
| 一边不存在，另一边存在 | 从存在的一方复制到缺失的一方 |
| 两边都存在但内容不同 | **比较内容丰富度**：行数、规则声明数量、示例数量、边界条件覆盖。更丰富的一方胜出，覆盖另一方。难以判定时暂停让用户选择。 |
| 文件名不一致（如 `SKILL.md` vs `skill.md`） | 统一为 `SKILL.md`（全大写），内容以更丰富的一方为准 |

**独有文件处理**：

| 情况 | 操作 |
|------|------|
| 仅存在于 `.agents/` | 报告，**不自动删除**，提示用户决定 |
| 仅存在于 `.claude/` | 报告，**不自动删除**，提示用户决定 |
...
| 冲突类型 | 策略 |
|----------|------|
| 同一文件两边内容不同 | **内容更丰富的一方胜出**（比较行数、规则详尽度、示例覆盖）。难以判定时暂停让用户选择 |
| 文件名大小写不一致 | 统一为 `SKILL.md`（全大写，vault 标准约定） |
| 仅存在于某一边的文件 | 报告，不自动删除，让用户决定 |
...
| Skills 表中的路径不一致 | 以两边实际存在的文件路径为准（检查磁盘上的真实文件） |
