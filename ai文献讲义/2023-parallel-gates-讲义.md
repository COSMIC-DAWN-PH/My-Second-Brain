...
1. [引言：为什么需要高保真纠缠门](#1-引言为什么需要高保真纠缠门)
2. [中性原子量子计算平台介绍](#2-中性原子量子计算平台介绍)
3. [量子比特编码：超精细 qubit](#3-量子比特编码超精细-qubit)
4. [Rydberg 态与 Rydberg blockade 物理](#4-rydberg-态与-rydberg-blockade-物理)
5. [两比特 CZ 门的基本原理](#5-两比特-cz-门的基本原理)
6. [核心创新一：时间最优单脉冲门](#6-核心创新一时间最优单脉冲门)
7. [核心创新二：暗态与亮态——如何抑制散射](#7-核心创新二暗态与亮态如何抑制散射)
8. [核心创新三：实验技术升级（冷却与光泵浦）](#8-核心创新三实验技术升级冷却与光泵浦)
9. [门表征方法（Bell 态、随机基准化）](#9-门表征方法bell-态随机基准化)
10. [规模化：60 比特并行操作](#10-规模化60-比特并行操作)
11. [多比特扩展：CCZ 门与 GHZ 态](#11-多比特扩展ccz-门与-ghz-态)
12. [误差来源分析与迈向 99.9%](#12-误差来源分析与迈向-999)
13. [总结与历史意义](#13-总结与历史意义)
14. [延伸阅读](#14-延伸阅读)
...
想象一支交响乐团演奏量子算法。弦乐、铜管、打击乐分别代表不同的量子比特。如果有一位乐手时不时地奏错音（退相干误差），整个演出就会失真。量子计算中的**纠缠门**就像乐团中的"二重奏"——它是最难演奏好的部分，也是最容易引入错误的环节。

**量子计算的基本门槛**：要运行有实用价值的量子算法（如 Shor 算法或量子模拟），错误率必须足够低。在没有量子纠错的情况下，错误率每降低一个数量级，可靠计算的电路深度就能增加约 10 倍。
...
**动机二：资源开销最小化**
量子纠错的资源开销是惊人的：要实现一个逻辑比特，通常需要数百甚至上千个物理比特。如果每个物理门的保真度能从 $99.0\%$ 提升到 $99.9\%$，所需的物理资源可能减少 10 倍。

**动机三：深度电路的需求**
如我们在 [[深度电路执行 (Deep-Circuit Execution)]] 中所见，执行深层量子电路需要在逻辑层面维持恒定熵。如果每层门的错误率为 $0.5\%$，50 层后错误累积将高达 $22\%$。高保真纠缠门是深度电路运行的必要条件。

### 具体怎么实现？

本文（2023-parallel gates）的核心贡献是：利用**里德堡阻塞**（Rydberg Blockade）结合**单脉冲时间最优控制**，在中性原子平台上实现了**高保真（>99.5%）、并行化**的两比特 CZ 门，且可以同时操控数十个量子比特对而不互相干扰。
...
中性原子的核心优势是**天然的可扩展性**：光镊阵列可以通过全息术或声光偏转器（AOD）快速生成任意二维图案，理论上可以扩展到数千个量子比特。
...
**步骤三：量子门操作**
使用微波或窄带激光对单个原子执行单比特门（基于[[拉比振荡 (Rabi Flopping)]]），使用里德堡激发实现两比特纠缠门。
...
可以将原子能级类比为一座多层建筑：
- 每层楼有不同的"主量子数" $n$（对应电子的能量层）
- 每层楼内部又因为"自旋-轨道耦合"分裂成多个"房间"（超精细能级）
...
**动机二：与里德堡态的耦合兼容性**
从超精细基态 $|1\rangle$ 到里德堡态 $|r\rangle$ 的跃迁可以用特定频率的激光驱动，这是实现[[里德堡阻塞 (Rydberg Blockade)]]的基础。
...
从 $|1\rangle$ 到里德堡态的激发则使用**双光子跃迁**：先用一束"耦合光"将 $|1\rangle$ 与一个中间态 $|e\rangle$ 耦合，再用一束"驱动光"将 $|e\rangle$ 与里德堡态 $|r\rangle$ 耦合。通过调节两束光的频率差，可以消除中间态的布居，实现高效的里德堡激发。
...
**动机一：相互作用极强**
里德堡原子的偶极矩按 $\sim n^2$ 增长，两个里德堡原子之间的范德华相互作用按 $\sim n^{11}$ 增长！在 $n \sim 70$ 时，即使两个原子相隔 5–10 μm（约为头发丝直径的 1/20），相互作用也能达到 MHz 量级。

**动机二：阻塞效应**
如果两个原子靠得很近，当一个原子被激发到里德堡态时，它产生的强电场会**改变相邻原子的能级结构**，使相邻原子的 $|1\rangle \to |r\rangle$ 跃迁频率发生显著偏移（AC Stark 移动）。如果激光频率恰好与单原子的跃迁共振，此时相邻原子的跃迁就**失谐了**，无法被激发到里德堡态。这就是**里德堡阻塞（Rydberg Blockade）**。
...
我们已经知道 [[CZ门 (CZ Gate)]] 的作用是在 $|11\rangle$ 态上施加一个 $\pi$ 相位：$|11\rangle \to -|11\rangle$，而其他三个计算基态保持不变。

在里德堡体系中，CZ 门的实现思路非常巧妙：

**第一步（π 脉冲）**：将控制原子 A 从 $|1\rangle$ 激发到里德堡态 $|r\rangle$。由于里德堡阻塞，目标原子 B 无法被激发到 $|r\rangle$。
...
**物理直觉**：整个过程中，$|11\rangle$ 态经历的"路径"与其他三个基态不同——它多了一段原子 A 在 $|r\rangle$ 而 B 被阻塞的时间。在这段时间里，由于原子间的相互作用，B 的 $|1\rangle$ 态会获得一个额外的几何相位（类似于 Aharonov-Bohm 效应中的相位）。
...
在 $|1\rangle \to |r\rangle$ 的双光子激发过程中，系统必须经过一个**中间态** $|e\rangle$。如果激光功率不稳或频率失谐不当，原子可能会"卡"在 $|e\rangle$ 上并自发辐射光子——这就是**散射误差**。
...
**动机一：过程层析术的局限性**
完整的量子过程层析（Quantum Process Tomography）可以给出量子通道的完整描述（$4^n \times 4^n$ 个参数，$n$ 为比特数），但对于两比特门，这种方法需要 **$n=2$ 时 256 次测量，统计误差大，且对局部误差敏感**。
...
**步骤**：
1. 生成 $m$ 个门的随机序列，每个门选自 Clifford 群
...
- 并行两比特 CZ 门：错误率与单独执行时几乎相同（说明并行化没有引入额外串扰）
...
**动机一：容错计算的需要**
在量子纠错的表面码（[[表面码 (Surface Code)]]）方案中，CCZ 门是实现**逻辑 qubit 之间的非线性相互作用**所必需的。

**动机二：CCZ → GHZ 态的制备**
GHZ 态（Greenberger-Horne-Zeilinger 态）是多比特最大纠缠态：$|\text{GHZ}\rangle = \frac{1}{\sqrt{2}}(|0\cdots 0\rangle + |1\cdots 1\rangle)$。通过级联 CCZ 门和 Hadamard 门，可以高效制备 GHZ 态。
...
**中期（~3年）**：
- 实现更深的冷却（nK 量级）
- 引入 **魔术光阱（magic trap）** 技术，使里德堡态和基态受到相同的光势阱囚禁，消除运动效应
- 实施实时相位校准系统
