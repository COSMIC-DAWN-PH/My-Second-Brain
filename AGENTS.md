...
### 学习进度 Block Reference 的区间语义

在知识笔记中，`^YYMMDD` 和 `^nuYYMMDD` 标记**交替形成区间**，表示学习进度：

- **`^260604`**（无 `nu`）= 该标记**之前**到上一个标记之间的内容已学懂
- **`^nu260604`**（有 `nu`）= 该标记**之前**到上一个标记之间的内容没懂

每个标记管到上一个**相反类型**的标记为止，形成交替链：

```
文档开头 ──→ ^260604 ──→ ^nu260604 ──→ ^260604 ──→ 文档末尾
  已学          没懂           已学           待学习
```

> [!warning] 注意
> 不要在同一位置同时放 `^260604` 和 `^nu260604`——它们表示不同的区间含义。

### 链接到具体章节 `[[File#标题]]`

链接到目标笔记的某个 `##` / `###` 章节，点击后直接跳转到该章节位置：

```
[[English-Name#章节标题]]
```

- 章节标题文字必须与笔记中的标题**完全一致**（含标点、空格、中文）
- 适用于：讲义目录、学习计划中引用特定知识点、总结中定位到具体小节

示例：`[[2023-parallel-gates-handout#6. 核心创新一：时间最优单脉冲门]]` → 点击跳转到讲义的 §6

### 链接到章节 + 显示文本 `[[File#标题|文本]]`

跳转到章节但显示自定义文字，避免标题过长影响阅读：

```
[[English-Name#章节标题|显示文本]]
```
...
```
[[English-Name^block-id|显示文本]]
```

示例：`[[Single-Qubit-Gates#^321fbe|旋转门]]` → 点击跳转到 `^321fbe` 标记的位置，显示"旋转门"

> [!warning] 注意
> `#^block-id` 前面的 `#` 不能省略，否则 Obsidian 会把它当成别名而非锚点。
