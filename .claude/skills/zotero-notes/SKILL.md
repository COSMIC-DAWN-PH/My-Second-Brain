...
- 保留 vault 原有英文（论文标题、文件名、已有英文标题、字段值、命令名、标准缩写与既有术语）。
- 新增给用户阅读的文献处理结果、知识笔记正文、表格说明、总结与建议必须以中文为主；若新引入英文术语，应尽量附中文解释或中文翻译。
- Python 代码和 matplotlib 图表内部文字必须使用英文，包括代码注释、标题、坐标轴、legend 与 annotation，以避免 CJK 乱码或 glyph warning。
- 不要因为编码问题把笔记正文改成英文；应通过 UTF-8 写入、转义或脚本处理解决。
...
> 完整流程见 **[references/bidirectional-linking.md](references/bidirectional-linking.md)**（含知识笔记 → 文献笔记正向链接、文献笔记 → 知识笔记回链、链接完整性验证、双链书写约定；章节编号保留，供其它技能引用）。每次创建或更新知识笔记时，必须按该文件完成双向链接。

## 📝 更新记录

- 2026-08-05: 渐进式拆分——§6 双向链接工作流移至 references/bidirectional-linking.md，SKILL.md 保留 §1–§5 与入口
