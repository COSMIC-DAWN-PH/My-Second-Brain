...
最容易混淆的一点是：**两个 qubit 的态不是写成两个互不相干的列向量，而是写成四个联合基态的叠加**。

我们固定约定：第一个位置是 qubit A，第二个位置是 qubit B：

$$
|ab\rangle \equiv |a\rangle_A \otimes |b\rangle_B, \qquad a,b\in\{0,1\}
$$
...
| 写法 | 含义 | 读法 |
|---|---|---|
| $\vert 00\rangle$ | A 在 $\vert 0\rangle$，B 在 $\vert 0\rangle$ | zero-zero |
| $\vert 01\rangle$ | A 在 $\vert 0\rangle$，B 在 $\vert 1\rangle$ | zero-one |
| $\vert 10\rangle$ | A 在 $\vert 1\rangle$，B 在 $\vert 0\rangle$ | one-zero |
| $\vert 11\rangle$ | A 在 $\vert 1\rangle$，B 在 $\vert 1\rangle$ | one-one |
...
如果选定基底顺序为 $\{|00\rangle, |01\rangle, |10\rangle, |11\rangle\}$，它也可以写成四维列向量：
...
- $\alpha$：测量到 $00$ 的振幅，概率是 $|\alpha|^2$
- $\beta$：测量到 $01$ 的振幅，概率是 $|\beta|^2$
- $\gamma$：测量到 $10$ 的振幅，概率是 $|\gamma|^2$
- $\delta$：测量到 $11$ 的振幅，概率是 $|\delta|^2$

> [!tip] 记忆方法
> 双量子比特的四个基态就像两个开关的四种联合状态：00、01、10、11。量子态不是“到底是哪一种”，而是这四种可能性的复振幅叠加。

#### 从两个单 qubit 态乘出来

如果 qubit A 和 qubit B 各自处在单独的态：

$$
|\psi_A\rangle = a|0\rangle + b|1\rangle, \qquad
|\psi_B\rangle = c|0\rangle + d|1\rangle
$$
...
像普通乘法一样展开，但每一项要保留两个 qubit 的标签：

$$
= ac|00\rangle + ad|01\rangle + bc|10\rangle + bd|11\rangle
$$

也就是说，如果这个态是直积态，那么四个系数一定满足：
...
$$
|\psi_A\rangle = |+\rangle = \frac{|0\rangle+|1\rangle}{\sqrt{2}},\qquad
...
注意这里是 $|00\rangle+|10\rangle$，不是 $|00\rangle+|01\rangle$。因为第一个 qubit A 在叠加，第二个 qubit B 固定为 $|0\rangle$。

> [!warning] 常见错误：把 qubit 顺序写反
> $|10\rangle$ 的意思是”第一个 qubit 是 1，第二个 qubit 是 0”。如果你把 A/B 的顺序换了，整个列向量中 $\beta$ 和 $\gamma$ 的位置也会换。这就是为什么写双量子比特态时必须先声明基底顺序。

#### 单 qubit 回顾：Bloch 球参数化

还记得 [[Qubit-State-and-Superposition|单量子比特]] 的任意纯态可以写成：

$$
|\psi\rangle = \cos\frac{\theta}{2}|0\rangle + e^{i\phi}\sin\frac{\theta}{2}|1\rangle
$$

这里只有**两个**角度参数 $\theta, \phi$，对应 Bloch 球面上的一个点——两个实参数就完全确定了一个单 qubit 纯态（去掉了归一化和全局相位的自由度）。

那么问题来了：**两个 qubit 的态能不能也用类似的三角函数参数化？**

#### 双 qubit 的三角函数参数化

答案是：可以，但需要**更多角度**。

从 §1.1 我们知道，一个两 qubit 纯态有 **6 个实参数**（8 个实参数 − 归一化 1 − 全局相位 1）。所以类比 Bloch 球，我们需要一组三角函数来装下这 6 个自由度。

最自然的方案来自 [[#6. Schmidt 分解：纠缠的数学本质|Schmidt 分解]]——把纠缠”拆干净”，剩下的用每个 qubit 自己的 Bloch 球角度描述。

**Step 1：Schmidt 系数用一个角度参数化**

$$
\lambda_1 = \cos\chi, \qquad \lambda_2 = \sin\chi, \qquad \chi \in \left[0, \frac{\pi}{2}\right]
$$

其中 $\lambda_1^2 + \lambda_2^2 = \cos^2\chi + \sin^2\chi = 1$ 自动满足归一化。$\chi$ 控制纠缠的强度：

- $\chi = 0$：$\lambda_2 = 0$，直积态（无纠缠）
- $\chi = \pi/4$：$\lambda_1 = \lambda_2 = 1/\sqrt{2}$，最大纠缠

**Step 2：每个 qubit 的局部基用 Bloch 球角度描述**

$$
|u_1\rangle = \cos\frac{\theta_A}{2}|0\rangle + e^{i\phi_A}\sin\frac{\theta_A}{2}|1\rangle
$$

$$
|u_2\rangle = -\sin\frac{\theta_A}{2}|0\rangle + e^{i\phi_A}\cos\frac{\theta_A}{2}|1\rangle
$$

$$
|v_1\rangle = \cos\frac{\theta_B}{2}|0\rangle + e^{i\phi_B}\sin\frac{\theta_B}{2}|1\rangle
$$

$$
|v_2\rangle = -\sin\frac{\theta_B}{2}|0\rangle + e^{i\phi_B}\cos\frac{\theta_B}{2}|1\rangle
$$

这里 $\{|u_1\rangle, |u_2\rangle\}$ 是 qubit A 的一组正交基（互为 Bloch 球上的”对跖点”），$\{|v_1\rangle, |v_2\rangle\}$ 同理。

**Step 3：加入相对相位**

最后一个自由度——两个 Schmidt 项之间的**相对相位** $\varphi$：

$$
|\Psi\rangle = \cos\chi\,|u_1\rangle_A|v_1\rangle_B + e^{i\varphi}\sin\chi\,|u_2\rangle_A|v_2\rangle_B
$$

**完整参数化：6 个角度**

$$
\boxed{|\Psi\rangle = \cos\chi\,|u_1\rangle_A|v_1\rangle_B + e^{i\varphi}\sin\chi\,|u_2\rangle_A|v_2\rangle_B}
$$

| 角度 | 取值范围 | 物理含义 |
|------|---------|---------|
| $\chi$ | $[0, \pi/2]$ | 纠缠强度（$\chi = \pi/4$ 时最大纠缠） |
| $\theta_A$ | $[0, \pi]$ | qubit A 局部基的极角 |
| $\phi_A$ | $[0, 2\pi)$ | qubit A 局部基的方位角 |
| $\theta_B$ | $[0, \pi]$ | qubit B 局部基的极角 |
| $\phi_B$ | $[0, 2\pi)$ | qubit B 局部基的方位角 |
| $\varphi$ | $[0, 2\pi)$ | 两个 Schmidt 项的相对相位 |

> [!tip] 物理直觉
> 这个参数化的逻辑是：”先决定两个 qubit 之间**纠缠多强**（$\chi$），再决定各自的**局部旋转方向**（$\theta_A, \phi_A, \theta_B, \phi_B$），最后决定**相位关系**（$\varphi$）。”类比的话：$\chi$ 决定”纠缠的浓度”，$\theta, \phi$ 决定”每个 qubit 的朝向”，$\varphi$ 决定”纠缠中的相位差”。

**展开到计算基**

把 $|u_i\rangle, |v_i\rangle$ 的三角函数形式代入，再用张量积展开：
...
$$
\alpha = \cos\chi\cos\frac{\theta_A}{2}\cos\frac{\theta_B}{2} + e^{i\varphi}\sin\chi\cdot\sin\frac{\theta_A}{2}\sin\frac{\theta_B}{2}\cdot e^{i(\phi_A+\phi_B)}
$$

$$
\beta = \cos\chi\cos\frac{\theta_A}{2}\cdot e^{i\phi_B}\sin\frac{\theta_B}{2} - e^{i\varphi}\sin\chi\cdot\sin\frac{\theta_A}{2}\cdot e^{i\phi_B}\cos\frac{\theta_B}{2}
$$

$$
\gamma = \cos\chi\cdot e^{i\phi_A}\sin\frac{\theta_A}{2}\cos\frac{\theta_B}{2} - e^{i\varphi}\sin\chi\cdot e^{i\phi_A}\cos\frac{\theta_A}{2}\sin\frac{\theta_B}{2}
$$

$$
\delta = \cos\chi\cdot e^{i\phi_A}\sin\frac{\theta_A}{2}\cdot e^{i\phi_B}\sin\frac{\theta_B}{2} + e^{i\varphi}\sin\chi\cdot e^{i\phi_A}\cos\frac{\theta_A}{2}\cdot e^{i\phi_B}\cos\frac{\theta_B}{2}
$$

看起来复杂，但结构是清晰的：每一项都是 $\cos\chi$（直积部分）和 $\sin\chi$（纠缠部分）的线性组合，再乘以各 qubit 的 Bloch 球三角函数。

> [!warning] 对比单 qubit
> 单 qubit：$\cos(\theta/2)|0\rangle + e^{i\phi}\sin(\theta/2)|1\rangle$ → **2 个角度**，对应 Bloch 球面 $S^2$。
> 双 qubit：**6 个角度**，对应更复杂的几何空间（$[\pi/2] \times S^2 \times S^2 \times S^1$）。
> 多出来的 4 个自由度正是纠缠的来源——它们编码了两个 qubit 之间的量子关联。

**常见例子的参数化**

| 态 | $\chi$ | $\theta_A$ | $\phi_A$ | $\theta_B$ | $\phi_B$ | $\varphi$ |
|---|---|---|---|---|---|---|
| $\vert 00\rangle$ | $0$ | 任意 | 任意 | 任意 | 任意 | 任意 |
| $\vert \Phi^+\rangle$ | $\pi/4$ | $0$ | 任意 | $0$ | 任意 | $0$ |
| $\vert \Psi^+\rangle$ | $\pi/4$ | $\pi/2$ | $0$ | $\pi/2$ | $0$ | $0$ |

验证 $|\Phi^+\rangle$：$\chi = \pi/4$ 给出 $\lambda_1 = \lambda_2 = 1/\sqrt{2}$，$\theta_A = \theta_B = 0$ 给出 $|u_1\rangle = |v_1\rangle = |0\rangle$，$|u_2\rangle = |v_2\rangle = |1\rangle$，代入即得：

$$
\frac{1}{\sqrt{2}}|0\rangle_A|0\rangle_B + \frac{1}{\sqrt{2}}|1\rangle_A|1\rangle_B = \frac{1}{\sqrt{2}}(|00\rangle + |11\rangle) = |\Phi^+\rangle \quad \checkmark
$$

> [!info] 为什么用 Schmidt 分解做参数化？
> 这是最”干净”的双 qubit 参数化方案，因为它把纠缠的强度（$\chi$）和局部自由度（$\theta, \phi$）完全分离开。其他方案（比如直接写 $\alpha, \beta, \gamma, \delta$ 的三角函数）虽然也行，但纠缠信息会混在各角度之间，不如这个直观。


^nu260603
### 1.3 Tensor Product 结构
...
- **基底顺序**：默认按 $|00\rangle, |01\rangle, |10\rangle, |11\rangle$ 排列，所以列向量写作 $(\alpha,\beta,\gamma,\delta)^T$
...
- 2026-06-03: 新增"双 qubit 三角函数参数化"小节，基于 Schmidt 分解给出 6 角度参数化方案，类比单 qubit Bloch 球，含展开公式与常见态验证。
- 2026-06-03: 修正 Markdown 表格中的 ket 写法，将表格内 `$|00\rangle$` 改为 `$\vert 00\rangle$`，避免 `|` 被解析成表格分隔符。
- 2026-06-03: 补充“双量子比特具体怎么写”小节，明确 $|ab\rangle$ 的 qubit 顺序、四维列向量表示、张量积展开和具体例子。
