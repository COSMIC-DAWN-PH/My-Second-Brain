...
这个恒等式看似简单，但它**深刻地揭示了 H 门的基变换本质**，并且在 [[CZ-Gate|CZ 门与 CNOT 门的互相转换]] 中扮演核心角色。

> [!info] 线性代数深层理解
> $HXH = Z$ 的本质是**相似变换** $S^{-1}AS$（$S = H$）——Z 是 X 门在 X 基下的矩阵表示。完整讲解见 [[Basis-Transformation]]，包括：基变换定理的推导、为什么 $S^{-1} = S^\dagger$（酉基变换）、共轭转置的定义与物理含义。
...
Rz 门在中性原子平台中并不是通过直接绕 $z$ 轴旋转来实现的——实际上，原子的两个编码能级（$\vert 0\rangle$ 和 $\vert 1\rangle$）之间通常没有方便的相干驱动路径来绕 $z$ 轴旋转。取而代之的是一个更精巧的方案：**失谐激光脉冲 + AC Stark 效应**。

**基本原理**：将一束激光调谐到与 $\vert 0\rangle \to \vert e\rangle$ 跃迁有较大失谐 $\Delta$（$\Delta \gg \Omega$）。此时不会真正驱动跃迁（布居转移可忽略），但由于 AC Stark 效应，$\vert 0\rangle$ 和 $\vert 1\rangle$ 两个能级会经历**不同的光位移**（light shift）：

$$
\delta E_0 \neq \delta E_1
$$

经过脉冲持续时间 $t$ 后，两个能级积累的相对相位为：

$$
\varphi = \frac{(\delta E_1 - \delta E_0)\,t}{\hbar}
$$

这等效于施加了 $R_z(\varphi)$ 旋转：
...
> [!tip] 物理直觉
> 失谐激光就像一个"隐形的手"——它不会真正推动原子在两个态之间跳跃，但会给两个态分别"加一层相位霜"。两层霜的厚度不同，于是两个态之间就产生了相对相位差。控制激光开启的时间，就能精确控制这个相位差的大小。

**Virtual Z 门**：在实际实验中，$R_z$ 还有一个更高效的实现方式——**虚拟 Z 门**（virtual Z gate）。原理是：不真的发射一束新激光脉冲，而是直接**改变后续所有脉冲的相位参考系**。例如，要实现 $R_z(\varphi)$，只需把后续驱动脉冲的相位偏移 $-\varphi$ 即可。这样做的优势是：
- 不需要额外的物理脉冲时间
- 没有额外的噪声引入
- 精度只受限于相位控制器的分辨率

$R_z$ 门的完整物理机制（包括 AC Stark 效应的微观推导）见 [[AC-Stark-Effect#3. Rz 门的物理实现|Rz 门实现]]。
...
- [[Basis-Transformation]] — **基变换与相似变换**：$HXH = Z$ 的线性代数基础、共轭转置、酉矩阵
