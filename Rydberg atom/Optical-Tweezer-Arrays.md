...
> [!tip] 光镊 = 微观机械手
> 可以把光镊想象成一只**看不见的微型机械手**：激光焦点就是手指尖，原子就是被抓住的小零件。关键在于这只手只用"光"就能握住和搬运原子，无需物理接触——这在量子计算中至关重要。
...
这在 [[Deep-Circuit-Execution]] 中至关重要：
- 将两个逻辑码块的对应原子**并排放置**，实现 [[Transversal-Gate]]
- 计算后将含错误的原子移走，换入新初始化的低熵原子

> [!warning] 原子重排的时间代价
> 移动原子虽然不破坏量子态，但需要**微秒到毫秒量级**的重排时间。对于实时纠错循环来说，这段时间可能成为瓶颈。因此实验中常采用**并行重排**或**预排列**策略来减少延迟。
...
> [!info] 当前实验前沿：阵列规模的飞速增长
> 截至 2026 年，光镊阵列已实现 **~600–1000 个原子的确定性排列**（Harvard/QuEra 等团队）。这意味着可以同时编码数十个逻辑 qubit 的距离-5 表面码——容错量子计算的硬件基础正在快速成熟。
...
两个相邻被俘获原子之间的里德堡相互作用产生 **[[Rydberg-Blockade]]** 效应，是实现 [[CZ-Gate]] 等两比特门的核心机制。

## 6. 在容错量子计算中的角色

在 Bluvstein et al. 2026 的论文框架中，光镊阵列平台支持：
- **[[Surface-Code]]**：二维阵列天然匹配表面码的拓扑结构
- **[[QEC]]**：通过 [[Rabi-Flopping]] 和 CZ 门实现 syndrome 测量
- **[[Deep-Circuit-Execution]]**：利用可重构性实现逻辑横向隐形传态
