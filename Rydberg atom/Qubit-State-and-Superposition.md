...
**单独测量**（$z$ 基）：两个态都给出 50% 概率得到 0 或 1——**完全一样**。

但如果你沿 $x$ 轴测量（施加 H 门后再测量 $z$ 基）：
...
$$
H|{-i}\rangle = \frac{1}{\sqrt{2}}(|0\rangle - i|1\rangle) \quad \text{（仍然是叠加态）}
$$

> [!warning] 关键点
> **相对相位不影响单次测量的概率分布（在同一个基下），但它决定了在不同测量基下得到什么结果**。这就是为什么量子门（如 H 门、Z 门）需要精确控制相位——相位差异在后续操作中会被"放大"成可观测的概率差异。
...
> [!tip] 统一视角
> 所有这六个特殊态（$|0\rangle, |1\rangle, |+\rangle, |-\rangle, |{+i}\rangle, |{-i}\rangle$）的**概率分布完全相同**——沿 $z$ 轴测都是 50-50。它们的区别**只在于相位结构**，这使得它们在不同测量基下给出不同的确定结果。
>
> **相位就是量子信息的"密码"**——同样的概率分布，不同的相位结构，可以编码完全不同的量子信息。
