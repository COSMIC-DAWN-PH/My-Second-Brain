...
$$
H|{-i}\rangle = \frac{1}{\sqrt{2}}(|0\rangle - i|1\rangle) \quad \Rightarrow \quad \text{50\% 得到 0，50\% 得到 1}
$$

**结果完全不同！** $x$ 基测量成功区分了两个态。

> [!warning] 关键点
> **相对相位不影响同一基下的测量概率分布，但它决定了在不同测量基下得到什么结果**。这就是为什么量子门（如 H 门、Z 门）需要精确控制相位——相位差异在后续操作中会被"放大"成可观测的概率差异。
