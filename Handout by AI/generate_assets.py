...
# Default output directory (used when run without flags)
OUTPUT_DIR = pathlib.Path(r"C:\Personal Profile\Profile\ScienceResearch\Quantum Computing\Handout by AI")
ASSETS = [
    "rabi_oscillations.png",
    "rydberg_blockade.png",
    "gate_waveforms.png",
    "dark_state_physics.png",
]

# ==========================================
# 1. Plot Rabi Oscillations
# ==========================================
def plot_rabi_oscillations(output_dir=OUTPUT_DIR):
    ...
    path = os.path.join(str(output_dir), "rabi_oscillations.png")
...
# ==========================================
def plot_rydberg_blockade(output_dir=OUTPUT_DIR):
    ...
    path = os.path.join(str(output_dir), "rydberg_blockade.png")
    plt.savefig(path, dpi=200)
    plt.close()
    print(f"Generated: {path}")
...
# ==========================================
def plot_gate_waveforms(output_dir=OUTPUT_DIR):
    ...
    plt.tight_layout()
    path = os.path.join(str(output_dir), "gate_waveforms.png")
    plt.savefig(path, dpi=200)
    plt.close()
    print(f"Generated: {path}")
...
# ==========================================
def simulate_dark_state_physics(output_dir=OUTPUT_DIR):
    ...
    path = os.path.join(str(output_dir), "dark_state_physics.png")
    plt.savefig(path, dpi=200)
    plt.close()
    print(f"Generated: {path}")
...
def run_all(output_dir=OUTPUT_DIR):
    """Regenerate every asset this script owns into output_dir."""
    output_dir = pathlib.Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    plot_rabi_oscillations(output_dir)
    plot_rydberg_blockade(output_dir)
    plot_gate_waveforms(output_dir)
    simulate_dark_state_physics(output_dir)


def _verify():
    """Regenerate into a temp dir and byte-compare with committed PNGs."""
    failed = []
    with tempfile.TemporaryDirectory() as td:
        run_all(pathlib.Path(td))
        for name in ASSETS:
            committed = OUTPUT_DIR / name
            generated = pathlib.Path(td) / name
            if not committed.is_file():
                print(f"MISSING committed asset: {committed}")
                failed.append(name)
                continue
            if not generated.is_file():
                print(f"NOT PRODUCED by script: {name}")
                failed.append(name)
                continue
            if committed.read_bytes() == generated.read_bytes():
                print(f"OK: {name}")
            else:
                print(f"STALE: {name} differs from current script output")
                failed.append(name)
    if failed:
        print(f"\n{len(failed)} asset(s) out of date: {', '.join(failed)}")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Handout by AI plot assets.",
        epilog=(
            "Examples:\n"
            "  python generate_assets.py            # regenerate the 4 PNGs in place\n"
            "  python generate_assets.py --dry-run  # print what would be written, write nothing\n"
            "  python generate_assets.py --check    # byte-compare committed PNGs vs current output"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--check", action="store_true",
        help="Regenerate into a temp dir and compare with committed PNGs; exit 1 on mismatch.",
    )
    parser.add_argument(
        "--dry-run", action="store_true",
        help="Print the PNG paths that would be generated without writing anything.",
    )
    args = parser.parse_args()

    if args.dry_run:
        for name in ASSETS:
            print(f"Would generate: {OUTPUT_DIR / name}")
        return
