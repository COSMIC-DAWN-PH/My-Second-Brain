...
1. [[#1. 引言：为什么需要高保真纠缠门]]
2. [[#2. 中性原子量子计算平台介绍]]
3. [[#3. 量子比特编码：超精细 qubit]]
4. [[#4. Rydberg 态与 Rydberg blockade 物理]]
5. [[#5. 两比特 CZ 门的基本原理]]
6. [[#6. 核心创新一：时间最优单脉冲门]]
7. [[#7. 核心创新二：暗态与亮态——如何抑制散射]]
8. [[#8. 核心创新三：实验技术升级（冷却与光泵浦）]]
9. [[#9. 门表征方法（Bell 态、随机基准化）]]
10. [[#10. 规模化：60 比特并行操作]]
11. [[#11. 多比特扩展：CCZ 门与 GHZ 态]]
12. [[#12. 误差来源分析与迈向 99.9%]]
13. [[#13. 总结与历史意义]]
14. [[#14. 延伸阅读]]
...
本文（2023-parallel gates）的核心贡献是：利用**里德堡阻塞**（Rydberg Blockade）结合**单脉冲时间最优控制**，在中性原子平台上实现了**高保真（>99.5%）、并行化（parallelized/parallel）**的两比特CZ 门  ，且可以同时操控数十个量子比特对而不互相干扰。
...
## 4. Rydberg 态与 Rydberg blockade 物理
...
| 原子 A | 原子 B | 计算基态 | 激光作用 |
|---|---|---|---|
| $\lvert 1\rangle$ | $\lvert 0\rangle$ | $\lvert 10\rangle$ | A 可被激发到 $\lvert r\rangle$，但 B 处于 $\lvert 0\rangle$，不参与里德堡激发。 |
| $\lvert 1\rangle$ | $\lvert 1\rangle$ | $\lvert 11\rangle$ | A 被激发到 $\lvert r\rangle$ 后，B 的里德堡激发被阻塞。 |
| $\lvert 0\rangle$ | $\lvert 0\rangle$ | $\lvert 00\rangle$ | A 不参与里德堡激发。 |
| $\lvert 0\rangle$ | $\lvert 1\rangle$ | $\lvert 01\rangle$ | A 不参与里德堡激发，B 可按设计路径演化。 |
